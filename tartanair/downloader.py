'''
Author: Yorai Shaoul, Wenshan Wang, Manthan Patel
Date: 2025-10-24

This file contains the download class, which downloads the data from Airlab Storage or Hugging Face to the local machine.
'''
# General imports.
import os
# import sys
...
class TartanAirDownloader(TartanAirModule):
    ...
    def refine_parameters(self, env, difficulty, modality, camera_name, unzip, delete_zip, config):
        '''
        1. Load from config if provided
        2. Ensure all inputs are lists
        3. If any input list is empty, fill it with all available options
        '''
    ...
    def download_multi_thread(self, env = [], difficulty = [], modality = [], camera_name = [], config = None, unzip = False, delete_zip = False, max_failure_trial = 3, num_workers = 8, **kwargs):
        ...
        if num_workers <= 1:
            ...
            with ThreadPoolExecutor(max_workers=num_workers) as executor:
                ...
                for future in as_completed(futures):
                    suc, success_filelist = future.result()
                    all_success_filelist.extend(success_filelist)
                    suc = suc and suc
        ...
        if suc:
            ...
            fail_list = [ff for ff in zipfilelist if join(self.tartanair_data_root, ff) not in all_success_filelist]
            for ff in fail_list:
                print_warn(ff)

        if unzip:
            self.unzip_files(all_success_filelist, remove_after = delete_zip)
...
class TartanGroundDownloader(TartanAirDownloader):
    ...
    def refine_parameters(self, env, version, traj, modality, camera_name, unzip, delete_zip, config):
        ...
        return env, version, traj, modality, camera_name, unzip, delete_zip
    ...
    def unzip_files(self, zipfilelist, remove_after = False):
        ...
        for zipfile_path in zipfilelist:
            ...
            filepath = os.path.dirname(zipfile_path)

            # os.makedirs(dest_dir, exist_ok=True)
            print(f"📦 Unzipping {filename} -> {filepath}")
            cmd = f'unzip -q -o "{zipfile_path}" -d "{filepath}"'
            os.system(cmd)

            if remove_after:
                cmd = f'rm "{zipfile_path}"'
                os.system(cmd)
                print(f"🗑️  Removed {zipfile_path}")

        print_highlight("✅ Unzipping Completed!")

    # def download_single_thread(self, env = [], version = [], traj = [], modality = [], camera_name = [], config = None, unzip = False, max_failure_trial = 3, **kwargs):
    #     """
    #     Downloads a trajectory from the TartanAir dataset. A trajectory includes a set of images and a corresponding trajectory text file describing the motion.
    ...
    def download_multi_thread(self, env = [], version = [], traj = [], modality = [], camera_name = [], config = None, unzip = False, delete_zip = False, max_failure_trial = 3, num_workers = 1, data_source = "huggingface", **kwargs):
        """
        ...
        if data_source == 'airlab':
            ...
            self.downloader = HuggingfaceDownloader(bucket_name = 'tartanground')

        # Refine parameters
        env, version, traj, modality, camera_name, unzip, delete_zip = self.refine_parameters(env, version, traj, modality, camera_name, unzip, delete_zip, config)
        ...
        if num_workers <= 1 or data_source == 'huggingface':
            ...
            with ThreadPoolExecutor(max_workers=num_workers) as executor:
                ...
                for i, zip_chunk in enumerate(file_chunks):
                    ...
                    future = executor.submit(self.download_single_thread, zip_chunk, max_failure_trial)
